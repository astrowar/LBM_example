    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            for (int a = 0; a < Q; a++) {
                ...
                real edotu = ex[a] * ux[index] + ey[a] * uy[index];
                real udotu = ux[index] * ux[index] + uy[index] * uy[index];
   ...
   //     [&](tbb::blocked_range<int> i_range) {   for (int i = i_range.begin(); i < i_range.end(); ++i)
    ...
    for (int i = 1; i < N - 1; i++) 
    {
        ...
        for (int j = 1; j < N - 1; j++)
        {
            ...
            real kinematicViscosity = LID_VELOCITY * (real)N / REYNOLDS_NUMBER;
            ...
            for (int a = 0; a < Q; a++) {
                ...
                if ((iS == 0) || (iS == N - 1) || (jS == 0) || (jS == N - 1)) {
                    // bounce back
                    real ubdote = ux[index_nbr] * ex[a] + uy[index_nbr] * ey[a];
